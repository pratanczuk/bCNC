...
class LibraryDialog(tk.Toplevel):
    ...
    def refresh(self, kind, selected=None):
        ...
        for name in sorted(self.library.records[kind]): listing.insert('end',name)
        if selected:
            listing.selection_set(sorted(self.library.records[kind]).index(selected))
            self.select(kind)
    ...
    def tool_changed(self):
        ...
        if pen: self.color_button.pack(side='left')
