...
class Segment:
    ...
    def correct(self):
        if self.type == Segment.LINE:
            return  # There's no use for this on lines
        ...
        if abs(self.startPhi) < EPS:
            self.startPhi = 0.0
        if abs(self.endPhi) < EPS:
            self.endPhi = 0.0
    ...
    def __repr__(self):
        ...
        if self.type == Segment.LINE:
            ...
            return "{} {} {}{} C:{} R:{:g} Phi:[{:g}..{:g}] L:{:g}".format(
                Segment._TYPES[self.type - 1],
                repr(self.A),
                repr(self.B),
                c,
    ...
    # ----------------------------------------------------------------------
    # Return a point ON the segment (or extrapolated outside of it) at
    # distance traveled from A (or B)
    # ----------------------------------------------------------------------
    def extrapolatePoint(self, dist, B=False):
        if self.type == Segment.LINE:
            ...
            # Handle circle/ARC with zero radius
            # (where did we get such segment???)
            if self.radius == 0:
                return self.B
            ...
            return Vector(
                self.C[0] + self.radius * cos(phi),
                self.C[1] + self.radius * sin(phi)
            )

    # ----------------------------------------------------------------------
    # Return a point ON the segment at distance traveled
    # from A to B (or B to A when negative)
    # ----------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # Return True if 2 Vectors have the same type, the same
        ...
        if splitlines or self.type == Segment.CW or self.type == Segment.CCW:
            ...
            for i in range(0, count):
                linearized.append(
                    Segment(
                        Segment.LINE,
                        self.distPoint(i * step),
                        self.distPoint((i + 1) * step),
                    )
                )
        else:
    ...
    def tangentStart(self):
        if self.type == Segment.LINE:
            ...
            if self.type == Segment.CCW:
                # return cross product -Oval x z(0,0,1)
                return Vector(-Oval[1], Oval[0])
            else:
                # return cross product -Oval x z(0,0,1)
                return Vector(Oval[1], -Oval[0])
    ...
    def _intersectLineLine(self, other):
        ...
        Dt = (
            -(other.A[0] - self.A[0]) * other.AB[1]
            + (other.A[1] - self.A[1]) * other.AB[0]
        )
        t = Dt / DD
        P = self.AB * t + self.A
        if (
            self.minx <= P[0] <= self.maxx
            and other.minx <= P[0] <= other.maxx
            and self.miny <= P[1] <= self.maxy
            and other.miny <= P[1] <= other.maxy
        ):
            return P, None
        return None, None
    ...
    def _intersectLineArc(self, arc):
        ...
        if t1 < -EPS or t1 > 1.0 + EPS:
            ...
            # P1 = AB*t1 + self.A
            P1 = Vector(self.AB[0] * t1 + self.A[0],
                        self.AB[1] * t1 + self.A[1])
        ...
        if t2 < -EPS or t2 > 1.0 + EPS:
            ...
            # P2 = AB*t2 + self.A
            P2 = Vector(self.AB[0] * t2 + self.A[0],
                        self.AB[1] * t2 + self.A[1])
    ...
    def _intersectCircleCircle(self, other):
        ...
        CC = other.C - self.C
        d = CC.norm()
        if d <= EPS2 or d >= self.radius + other.radius:
            return None, None
        # x = (d**2 + self.radius**2 - other.radius**2) / (2.*d)
        if abs(d) < abs(self.radius):
            x = (self.radius**2
                 + (d + other.radius) * (d - other.radius)) / (2.0 * d)
        else:
            x = (d**2
                 + (self.radius + other.radius)
                 * (self.radius - other.radius)) / (2.0 * d)
    ...
    def intersect(self, other):
        ...
        if max(self.minx, other.minx) > min(self.maxx, other.maxx):
            return None, None
        if max(self.miny, other.miny) > min(self.maxy, other.maxy):
            return None, None
    ...
    def split(self, P):
        ...
        if self.type > Segment.LINE:
            ...
            # , self.radius, self.startPhi, new.startPhi)
            self.setCenter(self.C)
        else:
            self.calcBBox()
        return new
...
class Path(list):
    ...
    def __repr__(self):
        return "{}:\n\t{}".format(
            self.name,
            "\n\t".join([f"{int(i):3d}: {x}" for i, x in enumerate(self)]),
        )
    ...
    # (experimental, but seems to work better, than previous version)
    ...
    def _direction(self, closed=True):
        ...
        for segment in self:
            if segment.type == Segment.CW:
                cwarc += segment.length()
            if segment.type == Segment.CCW:
                cwarc -= segment.length()
    ...
    # ---------------------------------------------------------------------
    # Return a point ON the path at distance traveled from
    # A to B (or B to A when negative)
    # ---------------------------------------------------------------------
    def distPoint(self, dist):
        if dist < 0:
    ...
    # This not only SPLITs path to contours, it also takes unsorted segments
    # and JOINs them to closed loops if possible
    ...
    def markInside(self, path, setinside):
        for i, si in enumerate(self):
            if path.isInside(si.midPoint()):
                si._inside.append(setinside)
